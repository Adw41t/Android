    ...
    inner class BrowserTabFragmentDecorator {
        ...
        private fun createPopupMenu() {
            ...
            popupMenu.apply {
                ...
                onMenuItemClicked(view.addAllBookmarksPopupMenuItem) {
                    launch {
                        pixel.fire(AppPixelName.MENU_ACTION_ADD_ALL_BOOKMARK_PRESSED.pixelName)
                        viewModel.tabs.value?.forEach {
                            viewModel.bookmarkAllTabs(it)
                        }

                        Snackbar.make(browserLayout, getString(R.string.bookmarkAddedMessage), Snackbar.LENGTH_LONG)
                            .show()
                    }
                }
